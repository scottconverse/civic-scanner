...
A portable [Agent Skill](https://agentskills.io/specification) for a one-person city newsroom. The AI acts as the reporter across the **whole civic landscape** affecting a town: government, schools, housing, business, health, transportation, utilities, environment, culture, neighborhood groups, local reporting, and community discussion. It verifies consequential claims, ranks leads, and writes substantial, nearly finished drafts for the human editor. For public meetings, it logs substantive actions and reconciles recordings with agendas and minutes. The editor chooses what to edit, return for rewriting or deeper digging, hold, or kill, and makes final publication and legal decisions.

The default source town is Longmont, Colorado. Use `discover` to build a registry for another town. The seven modes are `daily-scan`, `full-pipeline`, `verify-only`, `research`, `revise`, `legal-threat`, and `discover`.
...
The editor desk recommends **editorial readiness tiers**, distinct from source tiers A/B/C and the newsworthiness score. Tier 1 has a nearly finished, sourced draft for direct editing. Tier 2 has a substantial developing packet with clear gaps and AI next checks. Tier 3 preserves possible stories for investigation. The full pipeline keeps the story drafts, adversarial work, action ledger, and receipts behind the tiered dashboard. The editor can send any item back with a draft or instructions using `revise {story ID}` for rewriting, more reporting, a Dark Signal Desk check, or a kill decision. The AI completes accessible reporting before asking the editor to make a call or visit; the editor may report when genuinely required.

...
That's it. No dependencies, no config files. Claude Code will auto-detect the skill on next launch.
...
|------|---------|-----------------|
| **Daily Scan** | `daily-scan` | Prioritized lead list from all source tiers |
| **Full Pipeline** | `full-pipeline` | Complete story packages through all 11 agents |
...
- Pipeline stats (scanned / advanced / killed / held / suppressed)
- Beat context (run number, thread watch list, suppression triggers)
- Source access limitations + blocked source intelligence
...
Full agent output for editorial reference:
- Complete story drafts with attribution chains
- Newsworthiness scoring tables with justifications
- Black Desk speculative signals
- 4-gate adversarial verification with full counter-narratives
- Completeness audit results
- First Amendment analysis
- Plain-language rewrites
- Originality verification results
- Source hygiene and headline audit
...
civic-scanner/
  ...
  references/
    ...
    longmont-beat-memory.json           Beat memory (Longmont)
